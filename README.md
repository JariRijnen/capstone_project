...
This project consists of an ETL pipeline, which is orchestrated by Airflow (locally) via two DAGs. The first DAG (spark_pre_process_airflow) uploads local files to S3 and preprocesses the data via Spark and a EMR cluster. The second, wildfire_dag turns data from S3 storage to a Redshift data model. 
